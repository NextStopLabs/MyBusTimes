    ...
    axios.get('http://localhost:8000/api/game/')
        .then(response => {
            ...
            if (files && Array.isArray(files)) {
                // Map the list to extract the name and link for rendering
                const games = files.map(file => ({
                    name: path.basename(file.file, '.json'),
                    link: file.link
                }));
                ...
                res.status(500).send('Error: Invalid response from the API');
            }
        })
        .catch(err => {
            console.error(err);
...
router.get('/:game/map', async (req, res) => {
    ...
    try {
        ...
        if (routesResponse.status !== 200 || destsResponse.status !== 200) {
            console.error(`API responded with status: ${routesResponse.status} and ${destsResponse.status}`);
            return res.status(500).send('Error fetching routes or destinations from the API');
        }
...
router.get('/:game/routes', async (req, res) => {
    ...
    const breadcrumbs = [{ name: 'Home', url: '/' }];
    breadcrumbs.push({ name: game, url: `/game/${game}`, className: 'default' });
    breadcrumbs.push({ name: 'Routes', url: `/map/${game}/routes`, className: 'default' });
...
router.get('/:game/findRoute', async (req, res) => {
    ...
    if (!startPoint || !endPoint) {
        return res.status(400).send('Both startPoint and endPoint must be provided');
    }
    ...
    try {
        ...
        function dfs(currentPoint, path = [], depth = 0, visited = new Set()) {
            ...
            if (currentPoint === endPoint && path.length > 0) {
                resultOutput.push(path.map(seg => formatRoute(seg.num, seg.from, seg.to)));
                return;
            }

            routeSegments.forEach(segment => {
                if (segment.from === currentPoint && !visited.has(segment.from + segment.to)) {
