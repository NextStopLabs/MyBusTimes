...
const SHAPES = [
    ...
    [
        "Stripe (3)",
        "linear-gradient(0deg,user_colour 50%,transparent 50%)",
        "linear-gradient(0deg,user_colour 50%,transparent 50%)"
    ],
    [
        "Stripe Swoop",
    ...
    [
        "Upper band",
        "linear-gradient(0deg,transparent 50%, user_colour 50% 65%,transparent 65%)",
        "linear-gradient(0deg,transparent 50%, user_colour 50% 65%,transparent 65%)",
    ],
    [
        "Upper Swoop",
...
function isDarkColour(hex) {
    ...
    if (lum < 0.25) return true;
    if (lum < 0.35) {
        const max = Math.max(rgb.r, rgb.g, rgb.b) / 255;
        const min = Math.min(rgb.r, rgb.g, rgb.b) / 255;
        const delta = max - min;
        const sat = max === 0 ? 0 : delta / max;
        if (sat < 0.6) return true;
        return false;
    }
    return false;
}
