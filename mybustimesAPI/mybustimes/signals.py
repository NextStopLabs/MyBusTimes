...
def track_fleet_changes(sender, instance, **kwargs):
    print(f"Tracking changes for fleet {instance.pk}")
    if not instance.pk:
        return

    try:
        old_instance = fleet.objects.get(pk=instance.pk)
    ...
    add_change("on_load", old_instance.on_load, instance.preserved)
